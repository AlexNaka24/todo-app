...
# GET all todos
@router.get("/mytodos", status_code=status.HTTP_200_OK)
async def read_all(db: db_dependency):
    return db.query(models.Todos).all()

# GET todos by id
@router.get("/mytodos/{todo_id}", status_code=status.HTTP_200_OK)
...
@router.get("/mytodos/priority/", status_code=status.HTTP_200_OK)
async def read_by_priority(db: db_dependency, todo_priority: int = Path(le=6, gt=0)):
...
# POST todos
@router.post("/mytodos/createtodo/", status_code=status.HTTP_201_CREATED)
...
# PUT todos
@router.put("/mytodos/updatetodo/{todo_id}", status_code=status.HTTP_200_OK)
...
# DELETE todos
@router.delete("/mytodos/deletetodo/{todo_id}", status_code=status.HTTP_200_OK)
